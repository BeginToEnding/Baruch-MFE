...
void BondMarketDataService::OnMessage(OrderBook<Bond>& data)
{
    ...
    // OrderBook<Bond> may be non-assignable; avoid operator=
    books.erase(cusip);
    auto it = books.emplace(cusip, data).first;

    // Use stored object to ensure stable lifetime for listeners
    OrderBook<Bond>& stored = it->second;

    // Update best bid/offer if available
    const auto& bidStack = stored.GetBidStack();
    const auto& offerStack = stored.GetOfferStack();
 ...
 *
 * - mid is expected to oscillate between 99 and 101 in 1/256 increments (handled by feeder/generator).
 * - topSpread is the top-of-book bid/offer spread (should oscillate 1/128 -> ... -> 1/32 -> ... -> 1/128).
 * - For deeper levels, spread widens from topSpread by the smallest increment (1/256) per level.
 *
 * Level sizes:
...
{
    // Product lookup: Treasury derives from Bond, usable as Bond
